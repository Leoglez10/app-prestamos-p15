# Spec: admin-auth (simplification delta for `login-solo-codigo`)

## Purpose
This delta simplifies the admin authentication flow on `/prestamo-rapido` to a single code input. The previous `admin-auth-prestamo-rapido` change introduced a full auth machine (codigo + PIN + 8h TTL + 15min idle lock); this delta strips that down to the minimum viable for a single-operator personal kiosk: type code, use, logout, repeat. The accountability chain in the data layer (the three new columns on `prestamos_rapidos_alumnos`) is preserved — every loan still records who authorized it.
